"""Utilities for hooking up oauth2 to BinderHub's database
implements https://oauthlib.readthedocs.io/en/latest/oauth2/server.html
"""
# Copy from https://github.com/jupyterhub/jupyterhub/blob/master/jupyterhub/oauth/provider.py
...
from .token_provider import BinderHubTokenProvider
...
class BinderHubRequestValidator(RequestValidator):
    ...
    def validate_grant_type(
        ...
        Method is used by:
            ...
            - Refresh Token Grant
        """
        app_log.debug("validate_grant_type: %s", grant_type)
...
def make_provider(session_factory):
    """Make an OAuth provider"""
    db = session_factory()
    validator = BinderHubRequestValidator(db)
    server = BinderHubOAuthServer(db, validator)
    return server, BinderHubTokenProvider(db)
